 ...
 */
@Dao
abstract class ScheduleCompositeDAO {
    @Transaction
    @Query("SELECT * FROM schedule")
    abstract fun getAsStream(): Flow<List<ScheduleCompositeEntity>>

    @Transaction
    @Query("SELECT * FROM schedule WHERE is_complete = :isComplete")
    abstract fun findByCompleteAsStream(
        isComplete: Boolean
    ): Flow<List<ScheduleCompositeEntity>>
}
