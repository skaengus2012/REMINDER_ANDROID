...
class ScheduleListDiffer(listUpdateCallback: ListUpdateCallback) {
    ...
    fun getCurrentList(): List<ScheduleListItem> = when (val current = state) {
    ...
    fun tryMove(fromPosition: Int, toPosition: Int): Boolean {
        val movingList: MutableList<ScheduleListItem>? = when (val current = state) {
...
private sealed class ScheduleListDifferState private constructor() {
    ...
    class Move(
        origin: List<ScheduleListItem>
    ) : ScheduleListDifferState() {
        var pendingSubmit: PendingSubmit? = null
        var movingList = mutableListOf<ScheduleListItem>().apply { addAll(origin) }
    }

    class Sync(
        val movingList: List<ScheduleListItem>,
        var pendingSubmit: PendingSubmit?
    ) : ScheduleListDifferState()
}

@JvmInline
private value class PendingSubmit(val items: List<ScheduleListItem>?)