 ...
 * @author Thalys
 */
class GetAllUserScheduleListResourcesFlowUseCaseTest {
    @Test
    ...
    fun `Given schedules, When collect, Then schedules sorted by completion status`() = runTest {
        ...
        useCase.invoke().test {
            ...
            assertThat(actualSchedules, equalTo(expectedUserScheduleListResources))
            cancelAndIgnoreRemainingEvents()
        }
    }
    ...
    fun `Given slow getScheduleListResources, When new schedules emitted, Then ignore invalidated value`() = runTest {
        ...
        val collected = mutableListOf<List<UserScheduleListResource>>()
        backgroundScope.launch(unconfinedTestDispatcher()) {
...
) = GetAllUserScheduleListResourcesFlowUseCase(
