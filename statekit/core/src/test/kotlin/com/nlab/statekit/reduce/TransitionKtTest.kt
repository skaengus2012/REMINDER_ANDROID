...
class TransitionKtTest {
    ...
    fun `Given changeable and non changeable Transition, When transitionTo, Then invoke changeable only`() {
        fun testChangeableTransitionOnlyInvokedWhenFronted(
            ...
            val actualState = TestCompositeTransition(changeable, nonChangeable).transitionTo(
                inputAction,
                inputState,
                AccumulatorPool()
            )
        ...
        testChangeableTransitionOnlyInvokedWhenFronted { expectedState ->
            TestNodeTransition { _, _ ->
                expectedState
            }
        }
        testChangeableTransitionOnlyInvokedWhenFronted { expectedState ->
            TestLifecycleNodeTransition { _, _, _ ->
                expectedState
            }
        }
    }
    ...
    fun `Given non changeable and changeable Transition, When transitionTo, Then invoke all`() {
        ...
        fun <T : TestTransition> testAllTransitionInvokedWhenEnded(
            ...
            val actualState = TestCompositeTransition(nonChangeable, changeable).transitionTo(
                inputAction,
                inputState,
                accumulatorPool
            )
        ...
        testAllTransitionInvokedWhenEnded(
            nonChangeableTransitionMock = { input ->
                mock<TestLifecycleNodeTransition> {
    ...
    fun `Given two non changeable transitions, When transitionTo, Then invoke all`() {
        ...
        val actualState = TestCompositeTransition(nonChangeable1, nonChangeable2).transitionTo(
            inputAction,
            inputState,
            AccumulatorPool()
        )
