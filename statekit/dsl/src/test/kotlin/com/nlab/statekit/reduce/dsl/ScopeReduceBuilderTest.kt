...
class ScopeReduceBuilderTest {
    ...
    fun `When actionScope with action type, Then launched effect optionally`() = runTest {
            ...
            testScopeScopeReduceBuilder(
                ...
                setupReduce = { mockEffect ->
                    if (isInputActionMatched) {
                        ...
                        actionScope<TestAction.Action2> {
                            effect { mockEffect.invoke() }
                        }
                    }
                },
        ...
        testActionScopeWithActionType(isInputActionMatched = true)
        testActionScopeWithActionType(isInputActionMatched = false)
    }

    @Test
    fun `When stateScope, Then launched effect optionally`() = runTest {
        testScopeScopeReduceBuilder(
            setupReduce = { mockEffect ->
                stateScope {
                    effect { mockEffect.invoke() }
    ...
    @Test
    fun `When stateScope with state type, Then launched effect optionally`() = runTest {
        suspend fun testActionScopeWithActionType(
            ...
            testScopeScopeReduceBuilder(
                ...
                setupReduce = { mockEffect ->
                    if (isInputStateMatched) {
                        stateScope<TestState.State1> {
                            effect { mockEffect.invoke() }
                        }
                    } else {
                        stateScope<TestState.State2> {
                            effect { mockEffect.invoke() }
                        }
                    }
                },
